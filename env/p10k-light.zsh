# ACTIVE STYLE: rainbow (powerline prompt with colorful backgrounds).
#
# To try a different built-in style, source one of these into your shell to
# preview it live (no changes on disk; `exec zsh` reverts):
#
#   source ~/powerlevel10k/config/p10k-lean.zsh          # minimal, colored text, no backgrounds
#   source ~/powerlevel10k/config/p10k-classic.zsh       # powerline segments with a subtle frame
#   source ~/powerlevel10k/config/p10k-rainbow.zsh       # colorful background blocks (this file's base)
#   source ~/powerlevel10k/config/p10k-pure.zsh          # ultra-minimal 2-line "Pure" look
#   source ~/powerlevel10k/config/p10k-lean-8colors.zsh  # lean, restricted to 8 colors
#   source ~/powerlevel10k/config/p10k-robbyrussell.zsh  # classic oh-my-zsh default look
#
# To make one the new default, copy it over this file and re-apply the local
# tweaks below (search for "LOCAL OVERRIDE"), e.g.:
#
#   cp ~/powerlevel10k/config/p10k-lean.zsh ~/Documents/projects/terminal-config/env/p10k-light.zsh
#
...
() {
  ...
  typeset -g POWERLEVEL9K_LEFT_PROMPT_ELEMENTS=(
    ...
    context                 # user@hostname (LOCAL OVERRIDE: shown first, always — matches saved screenshot look)
    dir                     # current directory
    vcs                     # git status
    # LOCAL OVERRIDE: single-line prompt — `newline` removed and `prompt_char`
    # omitted so input sits on the same line, right after the git segment
    # (no prompt symbol / green block).
  )
  ...
  typeset -g POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(
    ...
    # time                  # current time
    # LOCAL OVERRIDE: single-line prompt — right-prompt `newline` removed too.
  ...
  # LOCAL OVERRIDE: nerdfont mode for the branch glyph; the dir/folder icon is
  # blanked separately below (search POWERLEVEL9K_HOME_ICON) to keep dir icon-free.
  ...
  # LOCAL OVERRIDE: blank the folder icon so dir shows text-only even in nerdfont
  # mode (we keep nerdfont for the git branch glyph, but want no folder icon).
  ...
  # LOCAL OVERRIDE: show the nerdfont branch glyph before the branch name.
  ...
  # LOCAL OVERRIDE: always show context (user@hostname), not just on SSH/root.
  # The stock rainbow template blanks DEFAULT/SUDO context here; we leave it
  # shown to match the saved screenshot look.
  ...
  # LOCAL OVERRIDE: rainbow ships this as `off`; we keep `always` so past
  # prompts collapse to a minimal form (re-apply if you swap styles).
